...
public class LoadBalancerTest {
    final LoadBalancer lb = new ConsistentHashLoadBalancer();
    ...
    public void selectTest() {
        ...
        // 连续调用 3 次
        ServiceMetaInfo serviceMetaInfo = lb.select(requestParams, serviceMetaInfoList);
        System.out.println(serviceMetaInfo);
        Assert.assertNotNull(serviceMetaInfo);
        serviceMetaInfo = lb.select(requestParams, serviceMetaInfoList);
        System.out.println(serviceMetaInfo);
        Assert.assertNotNull(serviceMetaInfo);
        serviceMetaInfo = lb.select(requestParams, serviceMetaInfoList);
        System.out.println(serviceMetaInfo);
